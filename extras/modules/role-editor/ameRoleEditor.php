...
class ameRoleEditor extends amePersistentModule {
	...
	/**
	 * @var array[] Editable roles per user. [userId => [roleA => true, roleB => true, ...]]
	 */
	...
	public function __construct($menuEditor) {
		...
		add_filter('editable_roles', array($this, 'filterEditableRoles'), 10, 1);
	...
	protected function getAllCapabilities($users = array()) {
		...
		$capabilities = array_merge($capabilities, $this->getUserDefinedCaps());
	...
	public function filterEditableRoles($editableRoles) {
		...
		if ( isset($this->cachedEditableRoles[$user->ID]) ) {
			...
			foreach($editableRoles as $roleId => $role) {
				if ( isset($this->cachedEditableRoles[$user->ID][$roleId]) ) {
					$result[$roleId] = $role;
				}
			}
			return $result;
